...
/**
 * This example plugin loads a given workspace and clones top-level
 * relationships into this workspace.
 *
    ...
    Set<Relationship> findRelationshipsInWorkspace(File file) {
        ...
        try {
            ...
            for (Relationship relationship : workspace.getModel().getRelationships()) {
                if (isPersonOrSoftwareSystem(relationship.getSource())
                        && isPersonOrSoftwareSystem(relationship.getDestination())) {
